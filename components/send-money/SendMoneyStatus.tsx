...
interface SendMoneyStatusProps {
  location: string;
}

export function SendMoneyStatus({ location }: SendMoneyStatusProps) {
  ...
  async function checkStatus() {
    setLoading(true);
    try {
      const { data } = await axios.get(`/api/send-money/status?location=${encodeURIComponent(location)}`);
      setResult(data);
    } catch (err) {
